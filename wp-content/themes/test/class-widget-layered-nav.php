...
class Widget_Layered_Nav extends WC_Widget
{
  ...
  public function __construct()
  {
    ...
    $this->widget_description = esc_html('Display a list of attributes to filter products in your store.', 'woocommerce');
    $this->widget_id          = 'woocommerce_layered_nav';
    $this->widget_name        = esc_html('Filter Products by Attribute', 'woocommerce');
    parent::__construct();
  }
  ...
  {
    // List display.
    echo '<ul class="woocommerce-widget-layered-nav-list ' . str_replace('pa_', 'variations-', $taxonomy) . '">';
    ...
    foreach ($terms as $term) {
      ...
      if ($count > 0 || $option_is_set) {
        ...
        $term_html = '<a rel="nofollow" href="' . esc_url($link) . '"> <span class="checkbox"></span>' . esc_html($term->name) . '</a>';
      } else {
        $link      = false;
      ...
      if ($taxonomy == "pa_color") {
        if ($count > 0 || $option_is_set) {
          $bg_color   = get_term_meta($term->term_id, 'pa_color_picker', true);
          $bg_color   = $bg_color ? $bg_color : $term->slug;
          $term_html  = '<a rel="nofollow" href="' . esc_url($link) . '"> <span class="clrbox" style="background-color:' . $bg_color . ';"> </span> <span class="tooltip-label">' . esc_html($term->name) . '</span> </a>';
        }
        $term_html    .= ' '. apply_filters('woocommerce_layered_nav_count', '<span class="count">(' . absint($count) . ')</span>', $count, $term);

        echo '<li class="color ' . ($option_is_set ? 'chosen' : '') . '">';
        echo apply_filters('woocommerce_layered_nav_term_html', $term_html, $term, $link, $count);
        echo '</li>';
      } else {
        echo '<li class="woocommerce-widget-layered-nav-list__item wc-layered-nav-term ' . ($option_is_set ? 'woocommerce-widget-layered-nav-list__item--chosen chosen' : '') . str_replace('pa_', ' ', $taxonomy).'">';
